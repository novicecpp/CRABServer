...
function manage_transfers {
    ...
    if [[ -f task_process/transfers.txt ]]; then
        ...
        timeout 15m env PYTHONPATH=$PYTHONPATH:$RucioPath python3 task_process/RUCIO_Transfers.py
        else
        timeout 15m env PYTHONPATH=$PYTHONPATH:$RucioPath python3 task_process/FTS_Transfers.py
        fi
...
export RucioPath=/cvmfs/cms.cern.ch/rucio/x86_64/slc7/py3/current/lib/python3.6/site-packages/
