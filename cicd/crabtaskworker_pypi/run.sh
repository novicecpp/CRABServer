...
if [[ $SERVICE == TaskWorker ]]; then
    declare -A links=(
        ["current/TaskWorkerConfig.py"]="/data/srv/${SERVICE}/hostdisk/cfg/TaskWorkerConfig.py"
    )
    WORKDIR=/data/srv/TaskManager
elif [[ "${SERVICE}" == Publisher* ]]; then
    declare -A links=(
        ["current/PublisherConfig.py"]="/data/srv/${SERVICE}/hostdisk/cfg/PublisherConfig.py"
    )
