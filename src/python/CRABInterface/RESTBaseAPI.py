...
class RESTBaseAPI(DatabaseRESTApi):
    ...
    def query_load_all_rows(self, match, select, sql, *binds, **kwbinds):
        ...
        with MeasureTime(self.logger, modulename=__name__, label='query_load_all_rows', trace=trace):
            ...
            for row in rows:
                ...
                for i in range(len(new_row)):
                    if isinstance(new_row[i], cherrypy.request.db['handle']['type'].LOB):
                        ...
                        new_row[i] = tmp
                ret.append(new_row)
        self.logger.info("Size of data loaded from db: query_size=%d trace=%s", get_size(ret), trace)
